# Guacamole 输入法、鼠标与自动网络恢复 recovery4 多角度审计报告

## 总体结论

`1.6.0-recovery4` 可以作为**受控环境部署候选版**，但在真实 Chrome/Edge → Guacamole → RDP → Windows 微软拼音链路验收通过前，不应称为已完成生产认证的正式版本。

## 本次新增修复

| 严重度 | 发现 | 影响 | recovery4 处理 |
| --- | --- | --- | --- |
| 中 | 隧道约 1.5 秒无入站数据就直接显示网络不稳定 | 标签页后台限流、恢复或短暂抖动会闪现误导性提示 | 保留底层检测和 15 秒超时，仅对可见提示增加 3 秒确认窗口；后台隐藏并在返回后重新计时 |
| 中 | 隧道恢复传输后，旧会话仍可能保留画面处理或链路延迟 | 鼠标、键盘和画面持续滞后，刷新整页后暂时恢复 | 确认异常后提供连接级“重新连接”路径，仅重建受影响的客户端 |
| 中 | 单纯依赖手动重连，用户可能无法在高延迟状态下及时操作 | 卡顿持续，仍需人工刷新或等待 | 网络恢复稳定 5 秒后自动重建；第二次使用 10 秒退避，最多两次；传输中或用户选择保留时取消 |
| 中 | 每个浏览器 `mousemove` 都直接进入隧道发送队列 | 弱网时过期移动堆积，点击和松开长期排队 | 移动合并为约 30 Hz 的最新坐标；按钮状态先刷新坐标再立即发送，连接替换时清空旧移动 |
| 高 | 已确认异常的隧道只有恢复为 `OPEN` 后才会安排自动重连 | 持续半开或最终超时的连接仍需刷新整个页面 | 确认异常后即安排连接级重建；持续不稳定和随后断线不会取消待执行恢复 |
...
| 高 | 负载均衡连接组重新连接可能选择不同后端 | 自动修复后画面可能变成另一台机器或另一个会话 | 连接组仅保留手动恢复；直连和活动连接才允许自动重建 |

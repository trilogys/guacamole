# Guacamole 输入法、鼠标与整页恢复 recovery6 多角度审计报告

## 总体结论

`1.6.0-recovery6` 可以作为**受控环境部署候选版**，但在真实 Chrome/Edge → Guacamole → RDP → Windows 微软拼音链路验收通过前，不应称为已完成生产认证的正式版本。

## 本次新增修复

| 严重度 | 发现 | 影响 | recovery6 处理 |
| --- | --- | --- | --- |
| 中 | 隧道约 1.5 秒无入站数据就直接显示网络不稳定 | 标签页后台限流、恢复或短暂抖动会闪现误导性提示 | 保留底层检测和 15 秒超时，仅对可见提示增加 3 秒确认窗口；后台隐藏并在返回后重新计时 |
| 中 | 隧道恢复传输后，旧页面仍可能保留画面处理或链路延迟 | 鼠标、键盘和画面持续滞后，手动刷新整页后暂时恢复 | 右下角始终提供“刷新页面”；用户可在设置中选择延迟自动刷新整个页面 |
| 中 | 无条件自动刷新会打断操作或形成刷新循环 | 文件传输中断、页面反复重载、连接组重新选机 | 默认关闭；开启后使用 5/10 秒退避、每标签页最多两次、稳定一分钟重置，并保护活动传输 |
| 中 | 每个浏览器 `mousemove` 都直接进入隧道发送队列 | 弱网时过期移动堆积，点击和松开长期排队 | 移动合并为约 30 Hz 的最新坐标；按钮状态先刷新坐标再立即发送，连接替换时清空旧移动 |
| 高 | 已确认异常的隧道只有恢复为 `OPEN` 后才会安排恢复 | 持续半开或最终超时的连接仍需人工刷新整个页面 | 开启设置时，确认异常后即安排整页刷新；持续不稳定和随后断线不会取消待执行刷新 |
| 高 | WebSocket 隧道 ping 正常时，guacd、远端协议或画面队列仍可能严重积压 | 打开远端浏览器后页面保持“已连接”，但连续点击长期没有及时反馈 | 以最佳同步偏移和浏览器渲染统计识别超过 3 秒的积压；三次有意按下后 8 秒仍无及时同步才标记控制无响应 |
| 高 | 整页刷新会重新加载全部连接，负载均衡组可能选择不同后端 | 刷新后画面可能变成另一台机器或另一个会话 | 设置和连接菜单明确提示风险，默认关闭整页自动刷新 |
| 中 | 实时 `sync` 回调每约 5 秒执行完整画布展开、缩放、PNG 编码及本地历史写入 | 与远程浏览器新建标签页的全屏重绘碰撞时，放大主线程偶发卡顿 | 移除实时周期缩略图；只在首次连接和断开时保存预览 |
| 中 | 为控制看门狗常驻 5 秒逐帧显示统计 | 高帧率重绘期间持续增加统计数组维护和聚合开销 | 默认关闭；仅在有意点击后临时启用 1 秒窗口，响应或超时立即关闭 |
| 高 | 镜像升级或回退后浏览器/代理可能混用不同构建的 HTML 与 JavaScript | 登录页可用但连接界面异常，普通刷新或镜像回退后仍可能保留问题 | 所有恢复操作先发送无缓存重验证请求并清除旧构建刷新锁，成功、失败或超时均只重载一次 |
| 中 | 远程画布黑屏但隧道未被判定异常时，右下角警告可能不出现 | 用户缺少可发现的恢复入口 | 连接菜单始终显示“刷新页面”，等待首帧的状态窗口也提供相同操作 |
...
| 中 | 只监听 `visibilitychange` 和 `window.focus` | 页面缓存恢复或生命周期冻结/恢复可能遗漏恢复时机 | 增加 `freeze`、`resume`、`pagehide`、`pageshow`、全屏和指针锁监听 |
| 低 | 发布元数据中的补丁哈希未与实际分发补丁强制一致 | 审计信息可能陈旧 | 包验证脚本直接计算补丁 SHA-256 并与发布元数据比较 |

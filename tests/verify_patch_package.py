...
def main() -> int:
    ...
    require("ghcr.io/trilogys/guacamole_patch:1.6.0-recovery6" in readme_en and
            "ghcr.io/trilogys/guacamole_patch:1.6.0-recovery6" in readme_zh,
    ...
    required = [
        ...
        "reloadCurrentPage",
        "removeItem('reloadedFor')",
        "Cache-Control', 'no-cache",
        "$window.location.reload(true)",
        "HELP_REFRESH_PAGE_NOW",
        "queueAutoRefresh",
        "cancelAutoRefresh",
        ...
        "without generating a synchronous",
    ...
    for needle, label in [
        ...
        ("guacamole-web-smoke-", "Web 容器隔离冒烟测试"),
        ("<meta name=\"build\"", "Guacamole 首页构建标识验证"),
        ("docker logs", "Web 启动失败日志"),
        ("org.opencontainers.image.licenses=Apache-2.0", "OCI 许可证标签"),
        ("org.opencontainers.image.version=${GUACAMOLE_VERSION}-recovery6", "具名恢复版本标签"),
    ...
    require("ghcr.io/trilogys/guacamole_patch:1.6.0-recovery6" in compose,
    ...
    for needle, label in [
        ...
        ("Publish recovery6 from", "Actions 运行名称"),
        ("PACKAGE_TAG: 1.6.0-recovery6", "具名镜像标签"),
    ...
    require(metadata["end_to_end_rdp_validation"] is False,
            "不得把尚未完成的端到端验证标记为已完成")
    require(metadata["package_version"] == "1.6.0-recovery6",
            "发布元数据版本必须为 recovery6")
    ...
    require(metadata["default_image"] == "ghcr.io/trilogys/guacamole_patch:1.6.0-recovery6",
